...
for ftp in files_tide_pass:
    ...
    while True:
        ...
        if line_t == "":
            break
        if "lat,lon,st_lat,st_lon,1-3s,3-9s,9-27s,tsunami_h(m),date" in line_t:
            while True:
                line_t2 = ft.readline()
                if line_t2 == "":
                    break
                ...
                # 日時の一致するUDファイルがあれば・・
                    ...
                    for fumd in files_ud_match_date:
                        ...
                        fu.close()
                        # 震源と観測点が一致ならば・・
                        if float(data_t[0][0:4])-0.9 <= lat <= float(data_t[0][0:4])+0.9 and float(data_t[1][0:5])-0.1 <= lon <= float(data_t[1][0:5])+0.1:
                        # if (float(data_t[2][0:4])-0.2 <= st_lat <= float(data_t[2][0:4])+0.2) and (float(data_t[3][0:5])-1 <= st_lon <= float(data_t[3][0:5])+1):
                            # print("\n観測点が一致")
